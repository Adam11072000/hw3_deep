...
def sliding_window_attention(q, k, v, window_size, padding_mask=None):
    ...
    # ====== YOUR CODE: ======
    q_unfold = q.unfold(-2, window_size, 1)
    k_unfold = k.unfold(-2, window_size, 1)
    v_unfold = v.unfold(-2, window_size, 1)

    q_unfold_transposed = q_unfold.transpose(-1, -2)
    k_unfold_transposed = k_unfold.transpose(-1, -2)
    print("q_unfold_transposed shape: ", q_unfold_transposed.shape)
    print("k_unfold_transposed shape: ", k_unfold_transposed.shape)
    print("k_unfold shape: ", k_unfold.shape)
    print("q_unfold shape: ", q_unfold.shape)
    scores = torch.matmul(q_unfold, k_unfold_transposed)
    if padding_mask is not None:
        padding_mask_expanded = padding_mask.unsqueeze(-1) # make sure to change when handling multiple num heads.
        scores.masked_fill_(padding_mask_expanded, float('-inf'))
